...
router.get("/", async function (req, res, next) {
  try {
    ...
    const categoriaResult = await collection.findOne({
      type: "table",
      name: "Categoria",
    });
    ...
    animeData.forEach((anime) => {
      anime.categoria_nombre =
        categoriaMap.get(anime.categoria_id) || "Categoría no encontrada";
    });
  ...
  async function (req, res, next) {
    try {
      ...
      if (nombre === "" || sinopsis === "" || categoria_id === "" || !imagen) {
        ...
        req.flash(
          "error",
          "Por favor, introduce nombre, sinopsis, categoría y una imagen"
        );
      ...
      if (!errors) {
        ...
        const lastAnime = await collection.findOne(
          { type: "table", name: "Anime" },
          { sort: { _id: -1 } }
        );
        let lastId = 0;
...
router.get("/edit/:id", async function (req, res, next) {
  try {
    ...
    const anime = await collection.findOne({
      type: "table",
      name: "Anime",
      "data.id": id,
...
router.get("/:id", async function (req, res, next) {
  try {
    ...
    const categoriaResult = await collection.findOne({
      type: "table",
      name: "Categoria",
      "data.id": anime.categoria_id,
    });

    if (
      !categoriaResult ||
      !categoriaResult.data ||
      categoriaResult.data.length === 0
    ) {
    ...
    const categoria = categoriaResult.data.find(
      (item) => item.id === anime.categoria_id
...
router.get("/listar/categorias", async function (req, res, next) {
  try {
    ...
    const categorias = await collection.findOne({
      type: "table",
      name: "Categoria",
    });
    ...
    categorias.data.forEach((categoria) => {
      const animesCategoria = animes.data.filter(
        (anime) => String(anime.categoria_id) === String(categoria.id)
      );
...
router.get("/html/view", async function (req, res, next) {
  try {
    await client.connect(); // Conectar al cliente de MongoDB
    const animeData = await collection.findOne({
      type: "table",
      name: "Anime",
    }); // Consultar documentos en la colección Anime
    ...
    // Obtener datos de la tabla 'Categoria'
    const categoriaData = await collection.findOne({
      type: "table",
      name: "Categoria",
    });
    ...
    animeData.data.forEach((anime) => {
      ...
      anime.categoria_nombre = categoria
        ? categoria.nombre
